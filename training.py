...
EMBEDDING_DIR = 'fasttext-embedding/crawl-300d-2M-subword.vec'
...
test_label = np.load(open(BASE_DIR + "/test_label.npy", 'rb')).tolist()
...
embeddings_index = {}
f = io.open(EMBEDDING_DIR, encoding='utf-8')
for line in f:
    value = line.split()
    word = value[0]
...
for filter in filters:
    conv_layer = Conv1D(number_filters, filter, activation='relu')(embedding_sequences)
...
pool_layer5 = MaxPool1D(20)(conv_layer5)
# conv_layer6 = Conv1D(number_filters, 5, activation='relu')(pool_layer5)
# pool_layer6 = MaxPool1D(MAX_SEQUENCE_LENGTH - 5 + 1)(conv_layer6)
flatten = Flatten()(pool_layer5)
# dense = Dense(256, activation='relu')(flatten)
preds = Dense(7, activation='softmax')(flatten)
